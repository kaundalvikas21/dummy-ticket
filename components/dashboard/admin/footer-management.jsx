...
import { Search, Plus, Edit, Trash2, Phone, Mail, MapPin, Save, X, Upload, Globe, Link2, GripVertical, Facebook, Youtube, Instagram, Image as ImageIcon } from "lucide-react"
...
export function FooterManagement() {
  ...
  // Form data states
  const [logoData, setLogoData] = useState({ url: "", alt_text: "", company_name: "" })
  ...
  const deleteOldLogo = async (logoUrl) => {
    ...
    try {
      ...
      const urlMatch = logoUrl.match(/\/storage\/v1\/object\/public\/assets\/(.+)/)
      if (urlMatch && urlMatch[1]) {
        const filePath = encodeURIComponent(urlMatch[1])

        // Use admin storage API to delete the file
        const response = await fetch(`/api/admin/storage/upload?path=${filePath}`, {
          method: 'DELETE'
        })
  ...
  const handleLogoUpload = async (file) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error('Storage upload failed:', { status: response.status, error: errorData })
      ...
      toast({
        title: "Logo Uploaded",
        description: "Logo uploaded to storage. Click 'Save All Footer Information' to save to database."
      })
  ...
  const saveLogoOnly = async () => {
    try {
      ...
      if (!logoData.company_name?.trim()) {
        throw new Error("Company name is required. Please enter a company name.")
      }
  ...
  const saveDescription = async () => {
    try {
      ...
      const existingDescription = footerData.description && typeof footerData.description === 'object'
      ...
      toast({ title: "Success", description: "Description saved successfully" })
    } catch (error) {
  ...
  const saveAddress = async () => {
    try {
      ...
      const existingAddress = footerData.address && typeof footerData.address === 'object'
        ? footerData.address
        : null
  ...
  const saveRow1Data = async () => {
    try {
      ...
      // Delete old logo from storage if URL has changed
      if (originalData.url && originalData.url !== logoData.url && originalData.url.includes('/storage/v1/object/public/')) {
        console.log('Deleting old logo during save:', originalData.url)
        await deleteOldLogo(originalData.url)
      }
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.error || "Failed to save primary information")
      }
      ...
      toast({
        title: "Success",
        description: "All footer information saved successfully"
      })
  ...
  const saveLink = async () => {
    try {
        ...
        // Add new link
          ...
          body: JSON.stringify({
            ...
            data: {
              ...
              sort_order: footerData[`${linkFormData.section}_links`]?.length || 0
            }
          })
        })
      ...
      await fetchFooterData()
      toast({ title: "Success", description: linkFormData.id ? "Link updated successfully" : "Link saved successfully" })
    } catch (error) {
      toast({
        title: "Error",
  ...
  const saveContact = async () => {
    try {
      ...
      const payload = {
        ...
        extra_data: {
          icon_type: contactFormData.icon_type,
          country: contactFormData.country,
          link_type: contactFormData.link_type
        }
      }

      // Use PUT if editing existing record (has ID), otherwise POST
      const method = contactFormData.id ? "PUT" : "POST"
      if (contactFormData.id) {
        payload.id = contactFormData.id
          } else {
        payload.sort_order = footerData.contact_items?.length || 0
          }
      ...
      await fetchFooterData()
      toast({ title: "Success", description: contactFormData.id ? "Contact item updated successfully" : "Contact item saved successfully" })
    } catch (error) {
      toast({
        title: "Error",
  ...
  const saveSocial = async () => {
    try {
      ...
      const payload = {
        section: "social",
        title: socialFormData.name,
        content: socialFormData.url,
        extra_data: { icon_name: socialFormData.icon_name }
      }

      // Use PUT if editing existing record (has ID), otherwise POST
      const method = socialFormData.id ? "PUT" : "POST"
      if (socialFormData.id) {
        payload.id = socialFormData.id
              } else {
        payload.sort_order = footerData.social_links?.length || 0
              }
      ...
      await fetchFooterData()
      toast({ title: "Success", description: socialFormData.id ? "Social link updated successfully" : "Social link saved successfully" })
    } catch (error) {
      toast({
        title: "Error",
  ...
  const deleteItem = async () => {
    ...
    try {
      ...
      let section = ""
      if (selectedItem.href && !selectedItem.link_type && !selectedItem.icon_name) {
      ...
      const response = await fetch(`/api/footer?section=${section}&itemId=${selectedItem.id}`, {
        method: "DELETE"
      })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-8">
        {/* Row 1: Single Column Primary Information */}
        <div className="space-y-6">
          {/* Logo Management */}
          <Card>
            <CardHeader>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setLogoData(prev => ({ ...prev, company_name: e.target.value }))}
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          try {
                            ...
                            if (logoData.url && logoData.url.includes('/storage/v1/object/public/')) {
                              await deleteOldLogo(logoData.url)
                            }

                            // Clear logo URL from database using new structure
              
              const payload = {
                operation: "update_primary",
                section: "primary_info",
                data: {
                  company_name: logoData.company_name || "VisaFly",
              ...
              if (!response.ok) {
                const errorData = await response.json()
                console.error("Error clearing logo from database:", errorData)
                toast({
                  title: "Warning",
                  description: "Logo file deleted but failed to update database. Please refresh and save again.",
                  variant: "destructive"
                })
                return
              }
            ...
            // Update footerData state immediately to avoid caching issues
            setFooterData(prev => ({
              ...prev,
              logo: {
                ...prev.logo,
            ...
            // Optionally refresh data from server (with small delay to ensure database is updated)
                        setTimeout(async () => {
              await fetchFooterData()
                          }, 500)
      ...
      >
        Remove
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setLogoData(prev => ({ ...prev, alt_text: e.target.value }))}
        ...
        {/* Unified Save Button for Row 1 */}
        <div className="flex justify-center pt-4">
          <Button
            onClick={saveRow1Data}
            disabled={savingRow1 || uploadingLogo || !hasRow1Changes() || !logoData.company_name?.trim()}
            className="px-8 py-2 text-base font-medium"
            size="lg"
          >
            {savingRow1 ? "Saving..." : uploadingLogo ? "Uploading..." : "Save All Footer Information"}
          </Button>
        </div>
      </div>

      {/* Row 2: 2-Column Interactive Management Grid */}
            ...
            <CardHeader>
              <CardTitle className="flex justify-between items-center">
                ...
                <Button
                  size="sm"
                  onClick={() => {
                    setLinkFormData({ title: "", url: "", section: "company", id: null })
            ...
            <CardHeader>
              <CardTitle className="flex justify-between items-center">
                ...
                <Button
                  size="sm"
                  onClick={() => {
                    setLinkFormData({ title: "", url: "", section: "support", id: null })
                    setShowLinkDialog(true)
                  }}
                >
                  <Plus className="w-4 h-4 mr-2" />
                      ...
                      <div className="flex gap-2">
                        <Button
                          ...
                          onClick={() => {
                            ...
                            const rawContent = item.href.replace(/^(tel:|mailto:)/, '')
                            setContactFormData({
                              title: item.text || "",
                              content: rawContent || "",
                    ...
                    <div key={social.id} className="flex items-center justify-between p-2 border rounded">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-gray-500">{social.href}</div>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        <Button
      ...
      <Dialog open={showLinkDialog} onOpenChange={setShowLinkDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {linkFormData.id ? 'Edit' : 'Add'} {linkFormData.section === "company" ? "Company" : "Support"} Link
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <Button onClick={saveLink} disabled={savingLink} className="w-full">
      ...
      <Dialog open={showContactDialog} onOpenChange={setShowContactDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{contactFormData.id ? 'Edit' : 'Add'} Contact Information</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <Label>Type</Label>
              <Select
                ...
                onValueChange={(value) => {
                  const newFormData = {
                    ...
                    title: updateContactDisplayText({ ...contactFormData, link_type: value })
                  }
            ...
            {contactFormData.link_type === "tel" && (
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => {
                    const newFormData = { ...contactFormData, country: e.target.value }
                    setContactFormData({
                      ...newFormData,
            ...
            <div>
              <Label>{contactFormData.link_type === "tel" ? "Phone Number" : "Email Address"}</Label>
              <Input
                value={contactFormData.content}
                onChange={(e) => {
                  const newFormData = { ...contactFormData, content: e.target.value }
                  setContactFormData({
                    ...newFormData,
                    title: updateContactDisplayText(newFormData)
                  })
                }}
                placeholder={contactFormData.link_type === "tel" ? "+1-800-123-4567" : "info@visafly.com"}
              />
            </div>
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                This text will be displayed in the footer. Auto-filled based on your country and phone/email above.
              </p>
            </div>
            <Button onClick={saveContact} disabled={savingContact} className="w-full">
      ...
      <Dialog open={showSocialDialog} onOpenChange={setShowSocialDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{socialFormData.id ? 'Edit' : 'Add'} Social Media Link</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
            ...
            <div>
              <Label>Icon</Label>
              <Select value={socialFormData.icon_name} onValueChange={(value) =>
            ...
            <Button onClick={saveSocial} disabled={savingSocial} className="w-full">

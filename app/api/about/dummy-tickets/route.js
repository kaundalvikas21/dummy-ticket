...
export async function POST(request) {
  try {
    ...
    validateInput(body, ['title', 'content', 'status', 'content_type'])

    const { title, content, content_type = 'simple', status = 'active', sort_order = 1 } = body
    ...
    if (error) {
      ...
      return createAuthError('Failed to create dummy ticket', 500)
    }

...
export async function POST(request, { params }) {
  try {
    ...
    for (const translation of translations) {
      if (!translation.locale || !translation.question || !translation.answer ||
          translation.question.trim() === '' || translation.answer.trim() === '') {
        return NextResponse.json(
          { error: 'Each translation must have locale, question, and answer' },
          { status: 400 }
        )
      }
    }
    ...
    for (const translation of translations) {
      const { locale, question, answer } = translation
    ...
    console.error('Error in POST /api/faq-page/items/[id]/translations/batch:', error)
    return NextResponse.json(
